<?php
/**
 * TEG Twitter API Admin Settings Class
 *
 * @author   ThemeEgg
 * @category Admin
...
if ( ! class_exists( 'TEG_TA_Admin_Settings', false ) ) :

/**
 * TEG_TA_Admin_Settings Class.
 */
class TEG_TA_Admin_Settings {
	...
	public static function save() {
		...
		// Trigger actions
		do_action( 'teg_twitter_api_settings_save_' . $current_tab );
		do_action( 'teg_twitter_api_update_options_' . $current_tab );
		do_action( 'teg_twitter_api_update_options' );

		self::add_message( __( 'Your settings have been saved.', 'teg-twitter-api' ) );
		self::check_download_folder_protection();

		// Clear any unwanted data and flush rules
		delete_transient( 'teg_twitter_api_cache_excluded_uris' );
		TEGTApi()->query->init_query_vars();
		TEGTApi()->query->add_endpoints();
		wp_schedule_single_event( time(), 'teg_twitter_api_flush_rewrite_rules' );

		do_action( 'teg_twitter_api_settings_saved' );
	}
	...
	public static function output() {
		...
		do_action( 'teg_twitter_api_settings_start' );
		...
		$tabs = apply_filters( 'teg_twitter_api_settings_tabs_array', array() );
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'title':
					...
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_twitter_api_settings_' . sanitize_title( $value['id'] ) );
					}
					break;
				...
				case 'sectionend':
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_twitter_api_settings_' . sanitize_title( $value['id'] ) . '_end' );
					}
					echo '</table>';
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_twitter_api_settings_' . sanitize_title( $value['id'] ) . '_after' );
					}
					break;
				...
				case 'multi_select_countries' :
					...
					if ( ! empty( $value['options'] ) ) {
						...
						$countries = TEGTApi()->countries->countries;
					}
				...
				default:
					do_action( 'teg_twitter_api_admin_field_' . $value['type'], $value );
					break;
			}
		}
	}
	...
	public static function save_fields( $options, $data = null ) {
		...
		foreach ( $options as $option ) {
			 ...
			 */
			if ( has_action( 'teg_twitter_api_update_option_' . sanitize_title( $option['type'] ) ) ) {
				teg_ta_deprecated_function( 'The teg_twitter_api_update_option_X action', '2.4.0', 'teg_twitter_api_admin_settings_sanitize_option filter' );
				do_action( 'teg_twitter_api_update_option_' . sanitize_title( $option['type'] ), $option );
				continue;
			}
			 ...
			 * @since 1.0.0
			 */
			$value = apply_filters( 'teg_twitter_api_admin_settings_sanitize_option', $value, $option, $raw_value );

			/**
			 * Sanitize the value of an option by option name.
			 * @since 1.0.0
			 */
			$value = apply_filters( "teg_twitter_api_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
			 ...
			 * @deprecated 2.4.0 - doesn't allow manipulation of values!
			 */
			do_action( 'teg_twitter_api_update_option', $option );
		}
	...
	public static function check_download_folder_protection() {
		...
		$downloads_url   = $upload_dir['basedir'] . '/teg_twitter_api_uploads';
		$download_method = get_option( 'teg_twitter_api_file_download_method' );
